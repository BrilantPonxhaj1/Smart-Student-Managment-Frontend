    ...
    <v-card class="d-flex flex-column align-center justify-center px-6 py-8 mt-17 w-25 h-50 rounded-xl">
      ...
      <v-form v-model="form" @submit.prevent="onSubmit" class="w-100" style="max-width: 500px;">
        <v-text-field v-model="email" :readonly="loading" :rules="[required]" class="mb-2" label="Email"
                      clearable variant="outlined" append-icon=" " prepend-inner-icon="mdi-email-outline" rounded></v-text-field>
        <v-text-field v-model="password" :readonly="loading" :append-icon="showPassword ? 'mdi-eye-off' : 'mdi-eye'" :type="showPassword ? 'text' : 'password'" :rules="[required]" label="Password"
            placeholder="Enter your password" clearable variant="outlined" prepend-inner-icon="mdi-lock-outline" @click:append="showPassword = !showPassword" rounded></v-text-field>
        <v-btn :disabled="!form || loading" :loading="loading" color="black" size="large" type="submit"
               variant="elevated"
               block rounded>
          Sign In
        </v-btn>
